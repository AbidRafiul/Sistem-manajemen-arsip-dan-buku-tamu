...
router.post("/delete", async (req, res) => {
  ...
  try {
    ...
    await DB("mst_unit_kerja").whereIn("id_unit_kerja", oPayload.id).update({ status: 'deleted', updated_at: new Date() , tz: typeof req !== 'undefined' ? (req.context?.tz || req.headers?.['x-tz'] || 'Asia/Jakarta') : 'Asia/Jakarta'});
