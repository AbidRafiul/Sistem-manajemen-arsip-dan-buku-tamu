...
router.get("/:id_surat_keluar", async (req, res) => {
  try {
    const surat = await DB("trs_surat_keluar as tsk")
      ...
      .where("tsk.id_surat_keluar", req.params.id_surat_keluar)
      .first();
    ...
    const bodyText =
      ...
      [
        ...
        `Jenis Surat: ${surat.nama_jenis_surat || "-"}`,
        "",
        `Tujuan: ${surat.tujuan || "-"}`,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${String(surat.nomor_surat || "surat-keluar").replace(/[^\w.-]+/g, "_")}.docx"`
    );
