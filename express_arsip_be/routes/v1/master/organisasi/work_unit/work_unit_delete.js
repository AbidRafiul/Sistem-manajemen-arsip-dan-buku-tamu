...
router.post("/delete", async (req, res) => {
  ...
  try {
    ...
    await DB("mst_unit_kerja").whereIn("id_unit_kerja", oPayload.id).update({ status: 'deleted', updated_at: new Date() , zona_waktu: typeof req !== 'undefined' ? (req.context?.timezone || req.headers?.['x-timezone'] || 'Asia/Jakarta') : 'Asia/Jakarta'});
