...
export async function up(knex) {
  ...
  // 1. Tambah menu Kategori Dokumen (id_menu: 34) under Master Arsip (id_menu_induk: 29)
  const menuExists = await knex("mst_menu").where("id_menu", 34).first();
  if (!menuExists) {
    await knex("mst_menu").insert({
      id_menu: 34,
      id_menu_induk: 29,
      kode_menu: "MASTER_ARSIP_CATEGORIES",
  ...
  for (const roleId of rolesToGrant) {
    const permExists = await knex("mst_peran_menu")
      .where({ id_peran: roleId, id_menu: 34 })
      .first();
    if (!permExists) {
...
export async function down(knex) {
  ...
  // Hapus hak akses
  await knex("mst_peran_menu").where("id_menu", 34).del();

  // Hapus menu
  await knex("mst_menu").where("id_menu", 34).del();
