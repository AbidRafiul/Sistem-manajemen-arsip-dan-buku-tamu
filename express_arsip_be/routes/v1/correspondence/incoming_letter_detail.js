...
const incomingLetterDetail = async (req, res) => {
  try {
    ...
    const vaDispositions = await DB("trx_disposisi_surat as tld").leftJoin("mst_instruksi_disposisi as mdi", "tld.instruksi_disposisi_id", "mdi.instruksi_disposisi_id").select("tld.disposisi_surat_id", "tld.surat_masuk_id", "tld.disposisi_induk_id", "tld.dari_pengguna_id", "tld.kepada_pengguna_id", "tld.instruksi_disposisi_id", "mdi.nama_instruksi", "tld.instruksi", "tld.catatan_disposisi", "tld.batas_waktu", "tld.status", "tld.received_at", "tld.processed_at", "tld.completed_at", "tld.created_by", "tld.updated_by", "tld.created_at", "tld.updated_at").where("tld.surat_masuk_id", oPayload.surat_masuk_id).orderBy("tld.created_at", "desc");
