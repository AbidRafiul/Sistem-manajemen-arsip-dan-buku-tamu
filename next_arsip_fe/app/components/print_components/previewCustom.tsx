...
export default function PreviewCustom({
    ...
    const exportPDF = async (adjust: PrintPageSettingState) => {
        ...
        try {
            const rekapPDF = Array.isArray(dataRekap?.data)
                ? structuredClone(dataRekap.data)
                : [];
    ...
    return (
        <>
            {pdfOnly ?
                <PrintPageSettingWithoutExcel
                    adjustDialog={state.adjustDialog}
                    setAdjustDialog={(v) =>
                        setState((p) => ({ ...p, adjustDialog: v }))
                    }
                    handleAdjust={exportPDF}
                /> :
                <PrintPageSetting
                    adjustDialog={state.adjustDialog}
                    setAdjustDialog={(v) =>
            ...
            <Dialog
                visible={state.jsPdfPreviewOpen}
                onHide={() =>
                    setState((p) => ({ ...p, jsPdfPreviewOpen: false }))
                }
                modal
