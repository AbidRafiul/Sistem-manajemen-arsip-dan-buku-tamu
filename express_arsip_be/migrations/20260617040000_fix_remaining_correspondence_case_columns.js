...
export async function up(knex) {
  ...
  await changeColumn(knex, "trs_incoming_letters", "Subject", "subject", "VARCHAR(255) NOT NULL");
  await changeColumn(knex, "trs_incoming_letters", "Status", "status", "ENUM('baru','diproses','didisposisi','selesai') NOT NULL DEFAULT 'baru'");

  await changeColumn(knex, "trs_incoming_letter_files", "Status", "status", "ENUM('active','nonactive') NOT NULL DEFAULT 'active'");

  await changeColumn(knex, "trs_letter_dispositions", "Instruction", "instruction", "TEXT NULL");
  await changeColumn(knex, "trs_letter_dispositions", "Status", "status", "ENUM('baru','dibaca','diproses','selesai') NOT NULL DEFAULT 'baru'");

  await changeColumn(knex, "trs_incoming_letter_trackings", "Notes", "notes", "TEXT NULL");
}
