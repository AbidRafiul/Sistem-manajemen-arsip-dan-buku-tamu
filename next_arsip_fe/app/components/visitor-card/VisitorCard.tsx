...
export default function VisitorCard({ visitationData, showQR = true }: VisitorCardProps) {
    ...
    return (
        <div className="visitor-card-printable-area flex justify-center items-center p-3">
            <div className="visitor-card bg-white border border-slate-300 rounded-xl p-4 shadow-md text-slate-800 tracking-wide relative overflow-hidden" 
                ...
                <div className="flex items-center justify-between border-b border-slate-200 pb-2 mb-3 mt-1">
                    ...
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${
                        isInside ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-slate-100 text-slate-600 border border-slate-200'
                    }`}>
                ...
                <div className="grid grid-cols-12 gap-3 items-start h-[calc(100%-60px)]">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-1 gap-1 text-[10px] border-t border-slate-100 pt-1.5">
                                <div className="truncate">
                                    ...
                                    <span className="text-slate-700 font-bold">{visitationData.VisitPurposeName || 'Keperluan Dinas'}</span>
                                </div>
                                <div className="truncate">
                                    <span className="text-slate-400 font-medium">Bertemu:</span>{' '}
                                    <span className="text-slate-700 font-bold">{visitationData.Fullname || visitationData.HostName || '-'}</span>
                                </div>
                    ...
                    <div className="col-span-4 flex flex-col items-center justify-between h-full border-l border-dashed border-slate-200 pl-2 text-center">
                        <div className="flex items-center justify-center bg-white p-1 border border-slate-200 rounded-lg shadow-sm">
                            {showQR && qrSrc ? (
                                <img src={qrSrc} alt="Access QR Code" className="w-24 h-24 block" />
                            ) : (
                                <div className="w-24 h-24 flex items-center justify-center text-slate-300 bg-slate-50 text-[10px]">QR Hidden</div>
                            )}
                        </div>

                        <div className="w-full mt-1">
                            <div className="text-[11px] font-black tracking-widest text-slate-900 uppercase font-mono bg-slate-100 py-0.5 px-1 rounded border border-slate-200 truncate">
                                {visitationData.VisitCode || 'GUEST'}
                            </div>
