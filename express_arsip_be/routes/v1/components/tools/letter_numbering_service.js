...
export const CAKUPAN_SEQUENCE_OPTIONS = [
  "global",
  "per_jenis_surat",
  "per_unit_kerja",
  "per_jenis_surat_unit_kerja",
];
export const TAHAP_PENERBITAN_OPTIONS = [
  "saat_draft_dibuat",
...
export const validateNumberingFormat = (formatNomor) => {
  ...
  const unsupportedTokens = [...format.matchAll(/\{[A-Z0-9_]+\}/g)]
    .map((match) => match[0])
    .filter((token) => !NUMBERING_TOKENS.includes(token));
...
const padSequence = (value, digit) =>
  String(value).padStart(Number(digit) || 1, "0");
...
const getScopeKey = ({ cakupanSequence, jenisSuratId, unitKerjaId }) => {
...
export const getNumberingReferences = async (db, { jenisSuratId, unitKerjaId }) => {
  const [jenisSurat, unitKerja] = await Promise.all([
    jenisSuratId
      ? db("mst_jenis_surat").where("jenis_surat_id", jenisSuratId).first()
      : null,
    unitKerjaId
      ? db("mst_unit_kerja").where("id_unit_kerja", unitKerjaId).first()
      : null,
  ]);

  return { jenisSurat, unitKerja };
};
...
export const previewNomorSurat = async (
  db,
  { format_nomor, jumlah_digit, nomor, jenis_surat_id, id_unit_kerja, tanggal_surat }
) => {
  const formatError = validateNumberingFormat(format_nomor);
  if (formatError) throw new Error(formatError);

  const { jenisSurat, unitKerja } = await getNumberingReferences(db, {
...
const getNextNumberPreview = async (
  db,
  { config, jenisSuratId, unitKerjaId, tanggalSurat }
) => {
  ...
  const sequence = await db("trs_sequence_penomoran_surat")
    .where({
...
export const previewActiveNomorSurat = async (
  db,
  { jenisSuratId, unitKerjaId, tanggalSurat }
) => {
...
export const generateNomorSurat = async (
  ...
  let sequence = await trx("trs_sequence_penomoran_surat")
    .where({
      id_penomoran_surat: config.id_penomoran_surat,
      periode_key: periodeKey,
      cakupan_key: cakupanKey,
  ...
  if (!sequence) {
    try {
      await trx("trs_sequence_penomoran_surat").insert({
        ...
        tahun: ["bulanan", "tahunan"].includes(config.periode_reset)
          ? date.getFullYear()
          : null,
        periode_key: periodeKey,
        cakupan_key: cakupanKey,
    ...
    sequence = await trx("trs_sequence_penomoran_surat")
      .where({
        id_penomoran_surat: config.id_penomoran_surat,
        periode_key: periodeKey,
        cakupan_key: cakupanKey,
  ...
  await trx("trs_sequence_penomoran_surat")
    .where("id_sequence_penomoran_surat", sequence.id_sequence_penomoran_surat)
    .update({
      nomor_terakhir: nextNumber,
      updated_at: new Date(),
    });

  const { jenisSurat, unitKerja } = await getNumberingReferences(trx, {
    jenisSuratId,
    unitKerjaId,
  });
