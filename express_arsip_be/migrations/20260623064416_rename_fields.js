const withSafeRawAlter = (knex) => {
  ...
  const safeRaw = async (...args) => {
    ...
    try {
      ...
      const isAlterChange = /^\s*ALTER\s+TABLE\s+`?[\w]+`?.*\bCHANGE\s+/i.test(
        sql,
      );
...
export async function up(knex) {
  ...
  // 1. Table: user_navigation
  await knex.raw(
    "ALTER TABLE `user_navigation` CHANGE `id_pengguna` `id_pengguna` INT(11) NULL DEFAULT NULL",
  );

  // 2. Table: mst_pengguna_peran
  await knex.raw(
    "ALTER TABLE `mst_pengguna_peran` CHANGE `user_role_id` `id_peran_pengguna` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna_peran` CHANGE `user_id` `id_pengguna` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna_peran` CHANGE `role_id` `id_peran` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna_peran` CHANGE `is_primary` `peran_utama` TINYINT(4) NULL DEFAULT 0",
  );

  // 3. Table: mst_pengguna
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `user_id` `id_pengguna` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `fullname` `nama_lengkap` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `username` `nama_pengguna` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `email` `surel` VARCHAR(150) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `telp` `telepon` VARCHAR(45) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `password` `kata_sandi` VARCHAR(255) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `branch_id` `id_cabang` INT(10) UNSIGNED NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `division_id` `id_divisi` INT(10) UNSIGNED NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `department_id` `id_departemen` INT(10) UNSIGNED NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `position_id` `id_jabatan` INT(10) UNSIGNED NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `work_unit_id` `id_unit_kerja` INT(10) UNSIGNED NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `failed_login_attempts` `gagal_masuk` INT(11) NULL DEFAULT 0",
  );
  await knex.raw(
    "ALTER TABLE `mst_pengguna` CHANGE `last_login_at` `terakhir_login` DATETIME NULL DEFAULT NULL",
  );

  // 4. Table: mst_peran_menu
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `role_menu_id` `id_peran_menu` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `role_id` `id_peran` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `menu_id` `id_menu` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `can_view` `hak_lihat` TINYINT(4) NOT NULL DEFAULT 1",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `can_create` `hak_buat` TINYINT(4) NOT NULL DEFAULT 0",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `can_update` `hak_ubah` TINYINT(4) NOT NULL DEFAULT 0",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `can_delete` `hak_hapus` TINYINT(4) NOT NULL DEFAULT 0",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran_menu` CHANGE `can_approve` `hak_setuju` TINYINT(4) NOT NULL DEFAULT 0",
  );

  // 5. Table: mst_peran
  await knex.raw(
    "ALTER TABLE `mst_peran` ALGORITHM=INPLACE, CHANGE `role_id` `id_peran` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran` CHANGE `role_code` `kode_peran` VARCHAR(45) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran` CHANGE `role_name` `nama_peran` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_peran` CHANGE `description` `deskripsi` TEXT COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );

  // 6. Table: mst_positions
  await knex.raw(
    "ALTER TABLE `mst_positions` ALGORITHM=INPLACE, CHANGE `position_id` `id_jabatan` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
  ...
  // 8. Table: mst_menu (sudah memakai nama Indonesia)
  await knex.raw(
    "ALTER TABLE `mst_menu` ALGORITHM=INPLACE, CHANGE `id_menu` `id_menu` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` ALGORITHM=INPLACE, CHANGE `id_menu_induk` `id_menu_induk` INT(10) UNSIGNED NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` CHANGE `kode_menu` `kode_menu` VARCHAR(45) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` CHANGE `nama_menu` `nama_menu` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` CHANGE `jalur_menu` `jalur_menu` VARCHAR(255) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` CHANGE `ikon_menu` `ikon_menu` VARCHAR(100) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` CHANGE `urutan` `urutan` INT(11) NOT NULL DEFAULT 0",
  );
  await knex.raw(
    "ALTER TABLE `mst_menu` CHANGE `status_aktif` `status_aktif` TINYINT(4) NOT NULL DEFAULT 1",
  );

  // 9. Table: mst_divisi (Di gambar deskripsi-nya VARCHAR(45), bukan TEXT)
  await knex.raw(
    "ALTER TABLE `mst_divisi` ALGORITHM=INPLACE, CHANGE `division_id` `id_divisi` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_divisi` CHANGE `branch_id` `id_cabang` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_divisi` CHANGE `division_code` `kode_divisi` VARCHAR(45) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_divisi` CHANGE `division_name` `nama_divisi` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_divisi` CHANGE `description` `deskripsi` VARCHAR(255) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );

  // 10. Table: mst_departemen
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `department_id` `id_departemen` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `division_id` `id_divisi` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `department_code` `kode_departemen` VARCHAR(50) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `department_name` `nama_departemen` VARCHAR(150) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `description` `deskripsi` TEXT COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );

  // 11. Table: mst_cabang
  await knex.raw(
    "ALTER TABLE `mst_cabang` ALGORITHM=INPLACE, CHANGE `branch_id` `id_cabang` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_cabang` CHANGE `branch_code` `kode_cabang` VARCHAR(50) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_cabang` CHANGE `branch_name` `nama_cabang` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_cabang` CHANGE `address` `alamat` TEXT COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_cabang` CHANGE `telp` `telepon` VARCHAR(45) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_cabang` CHANGE `email` `surel` VARCHAR(150) COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );

  // 12. Table: mst_riwayat_audit
  await knex.raw(
    "ALTER TABLE `mst_riwayat_audit` CHANGE `username` `nama_pengguna` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_riwayat_audit` CHANGE `role` `peran` VARCHAR(50) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_riwayat_audit` CHANGE `aksi` `aksi` VARCHAR(100) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_riwayat_audit` CHANGE `ip_address` `alamat_ip` VARCHAR(50) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_riwayat_audit` CHANGE `agen_pengguna` `agen_pengguna` TEXT COLLATE utf8mb4_unicode_ci NOT NULL",
  );
...
export async function down(knex) {
  ...
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `id_departemen` `department_id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `id_divisi` `division_id` INT(10) UNSIGNED NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `kode_departemen` `department_code` VARCHAR(50) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `nama_departemen` `department_name` VARCHAR(150) COLLATE utf8mb4_unicode_ci NOT NULL",
  );
  await knex.raw(
    "ALTER TABLE `mst_departemen` CHANGE `deskripsi` `description` TEXT COLLATE utf8mb4_unicode_ci NULL DEFAULT NULL",
  );
