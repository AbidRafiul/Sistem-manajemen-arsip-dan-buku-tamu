...
router.post("/delete", async (req, res) => {
  ...
  try {
    ...
    await DB("mst_cabang").whereIn("id_cabang", oPayload.id).update({ status: 'deleted', updated_at: new Date() , tz: typeof req !== 'undefined' ? (req.context?.timezone || req.headers?.['x-timezone'] || 'Asia/Jakarta') : 'Asia/Jakarta'});
    ...
    if (deptIds.length > 0) {
      await DB("mst_departemen").whereIn("id_departemen", deptIds).update({ status: 'deleted', updated_at: new Date() , tz: typeof req !== 'undefined' ? (req.context?.timezone || req.headers?.['x-timezone'] || 'Asia/Jakarta') : 'Asia/Jakarta'});
      const divs = await DB("mst_divisi").whereIn("id_departemen", deptIds).select("id_divisi");
      const divIds = divs.map(d => d.id_divisi);
      if (divIds.length > 0) {
        await DB("mst_divisi").whereIn("id_divisi", divIds).update({ status: 'deleted', updated_at: new Date() , tz: typeof req !== 'undefined' ? (req.context?.timezone || req.headers?.['x-timezone'] || 'Asia/Jakarta') : 'Asia/Jakarta'});
        await DB("mst_unit_kerja").whereIn("id_divisi", divIds).update({ status: 'deleted', updated_at: new Date() , tz: typeof req !== 'undefined' ? (req.context?.timezone || req.headers?.['x-timezone'] || 'Asia/Jakarta') : 'Asia/Jakarta'});
      }
    }
