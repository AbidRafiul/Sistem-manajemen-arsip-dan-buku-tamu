export const apiEndpointDocumentGet = '/arsip-dokumen/get'
export const apiEndpointDocumentCreate = '/arsip-dokumen/create'
export const apiEndpointDocumentUpdate = '/arsip-dokumen/update'
export const apiEndpointDocumentDelete = '/arsip-dokumen/delete'
export const apiEndpointDocumentDetail = '/arsip-dokumen/detail'
export const apiEndpointDocumentPreview = '/arsip-dokumen/preview'

// Versioning
export const apiEndpointVersionUpload = '/arsip-dokumen/version/upload'
export const apiEndpointVersionDownload = '/arsip-dokumen/version/download'
export const apiEndpointVersionRollback = '/arsip-dokumen/version/rollback'
export const apiEndpointVersionApprove = '/arsip-dokumen/version/approve'
export const apiEndpointCategoryGet = '/arsip-dokumen/master/category'
export const apiEndpointConfidentialityGet = '/arsip-dokumen/master/confidentiality'
export const apiEndpointQrGenerate = '/arsip-dokumen/qr/generate'
export const apiEndpointQrScan = '/arsip-dokumen/qr/scan'
export const apiEndpointLocationUpdate = '/arsip-dokumen/location/update'
export const apiEndpointRetentionGet = '/arsip-dokumen/master/retention'

// OCR, Full-Text Search & Audit Trail
export const apiEndpointDocumentSearch = '/arsip-dokumen/search'
export const apiEndpointContentGet = '/arsip-dokumen/content/get'
export const apiEndpointOcrProcess = '/arsip-dokumen/ocr/process'
export const apiEndpointOcrStatus = '/arsip-dokumen/ocr/status'
export const apiEndpointHistoryGet = '/arsip-dokumen/history/get'
export const apiEndpointNumberGenerate = '/arsip-dokumen/number/generate'
