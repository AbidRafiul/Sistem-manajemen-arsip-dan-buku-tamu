 ...
 */
export const recordAuditTrail = async (nama_pengguna, peran, action, req, status = "SUKSES") => {
  try {
    const cIp = req?.headers?.["x-forwarded-for"] ? req.headers["x-forwarded-for"].split(",")[0].trim() : req?.ip || req?.connection?.remoteAddress || "Unknown";
    ...
    if (ipColumn) oPayload[ipColumn] = cIp;
    if (userAgentColumn) oPayload[userAgentColumn] = req?.headers?.["user-agent"] || "Unknown";
    await DB("mst_riwayat_audit").insert(oPayload);
  } catch (error) {
