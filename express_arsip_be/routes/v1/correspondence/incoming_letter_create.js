...
const getAgendaYear = () => new Intl.DateTimeFormat("en-US", {
...
const generateAgendaNumber = async trx => {
  ...
  const oLastAgenda = await trx("trs_surat_masuk").select("nomor_agenda").where("nomor_agenda", "like", `${cPrefix}%`).orderBy("nomor_agenda", "desc").forUpdate().first();
  const cLastSequence = String(oLastAgenda?.nomor_agenda || "").slice(cPrefix.length);
  const nLastSequence = /^\d+$/.test(cLastSequence) ? Number(cLastSequence) : 0;
  const cNextSequence = String(nLastSequence + 1).padStart(AGENDA_SEQUENCE_LENGTH, "0");
...
const incomingLetterCreate = async (req, res) => {
  try {
    ...
    const dNow = new Date();
    const nActorId = oPayload.created_by || req?.auth?.id_pengguna || req?.context?.id_pengguna || 1;
    const nIncomingLetterId = await DB.transaction(async trx => {
      const cNomorAgenda = oPayload.nomor_agenda || (await generateAgendaNumber(trx));
