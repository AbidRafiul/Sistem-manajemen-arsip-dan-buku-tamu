...
const Page = () => {
    ...
    const formik = useFormik({
        ...
        validate: (data: initValue) => {
            ...
            if (!data.kode_jenis_dokumen) {
                errors.kode_jenis_dokumen = 'Kode jenis dokumen wajib diisi';
            }
            if (!data.nama_jenis_dokumen) {
                errors.nama_jenis_dokumen = 'Nama jenis dokumen wajib diisi';
            }
            return errors;
        },
        onSubmit: async (data) => {
    ...
    const handleSave = async (input: initValue) => {
        ...
        try {
            const isEdit = Boolean(input.id_jenis_dokumen);
            if (isEdit) {
                const cEndPoint = `${apiEndpointUpdate}/${input.id_jenis_dokumen}`;
                const oBody = {
                    kode_jenis_dokumen: input.kode_jenis_dokumen,
                    nama_jenis_dokumen: input.nama_jenis_dokumen,
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            for (const item of state.selectedData) {
                const cEndPoint = `${apiEndpointDelete}/${item.id_jenis_dokumen}`;
