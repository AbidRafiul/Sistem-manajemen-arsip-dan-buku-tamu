...
const peranAliases = peran => {
  ...
  const cKey = normalized.toLowerCase();
  return Array.from(new Set([normalized, ...(peran_ALIASES[cKey] || [])].filter(Boolean)));
};
...
const normalizeLegacyMenu = rawMenu => {
  ...
  if (Array.isArray(parsed?.menus)) {
    ...
    return [grouped.home.length && {
      label: "HOME",
      items: grouped.home
    }, grouped.setup.length && {
      label: "SETUP",
      items: grouped.setup
    }, grouped.guest.length && {
      label: "BUKU TAMU",
      items: grouped.guest
    }, grouped.archive.length && {
      label: "ARSIP",
      items: grouped.archive
    }, grouped.report.length && {
      label: "REPORT",
      items: grouped.report
    }].filter(Boolean);
  }
  return [];
};
const buildTree = rows => {
  ...
  const clean = items => items.sort((a, b) => a.sortOrder - b.sortOrder).map(node => {
    const {
      items: childItems,
      ...item
    } = node;
...
const getPrimaryRole = async (DB, user) => {
  ...
  const query = DB("mst_pengguna_peran as pengguna_peran").leftJoin("mst_peran as peran", "pengguna_peran.id_peran", "peran.id_peran").select("peran.kode_peran", "peran.nama_peran").where("pengguna_peran.id_pengguna", user.id_pengguna).where(builder => {
    builder.where("pengguna_peran.status", "active").orWhereNull("pengguna_peran.status");
  }).orderBy("pengguna_peran.peran_utama", "desc");
  return await querySafely(() => query.first());
};
const getUser = async (DB, uniqueId) => {
  const query = DB("mst_pengguna").select("id_pengguna", "nama_pengguna", "nama_lengkap", "status");
  query.where(builder => {
...
const getRbacMenu = async (DB, user) => {
  ...
  let rows = [];
  if (user?.id_pengguna) {
    rows = (await querySafely(() => DB("mst_menu as menu").distinct("menu.id_menu as MenuId", "menu.id_menu_induk as ParentMenuId", "menu.nama_menu as MenuName", "menu.jalur_menu as MenuPath", "menu.ikon_menu as MenuIcon", "menu.urutan as SortOrder").join("mst_peran_menu as peran_menu", "peran_menu.id_menu", "menu.id_menu").join("mst_peran as peran", "peran.id_peran", "peran_menu.id_peran").join("mst_pengguna_peran as pengguna_peran", "pengguna_peran.id_peran", "peran.id_peran").where("pengguna_peran.id_pengguna", user.id_pengguna).where("menu.status_aktif", 1).where("peran_menu.hak_lihat", 1).where("peran.status", "active").where("pengguna_peran.status", "active").orderBy("menu.urutan", "asc"))) || [];
  }
  if (rows.length < 1 && user?.peran) {
    rows = (await querySafely(() => DB("mst_menu as menu").distinct("menu.id_menu as MenuId", "menu.id_menu_induk as ParentMenuId", "menu.nama_menu as MenuName", "menu.jalur_menu as MenuPath", "menu.ikon_menu as MenuIcon", "menu.urutan as SortOrder").join("mst_peran_menu as peran_menu", "peran_menu.id_menu", "menu.id_menu").join("mst_peran as peran", "peran.id_peran", "peran_menu.id_peran").where(builder => {
      builder.whereIn("peran.kode_peran", peranAliases(user.peran)).orWhereIn("peran.nama_peran", peranAliases(user.peran));
    }).where("menu.status_aktif", 1).where("peran_menu.hak_lihat", 1).where("peran.status", "active").orderBy("menu.urutan", "asc"))) || [];
  }
