...
router.post("/surat-keluar/:id_surat_keluar/finalisasi", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async trx => {
      ...
      const existingFlow = await trx("trs_alur_tanda_tangan").where("id_surat_keluar", oPayload.id_surat_keluar).first();
      if (existingFlow) {
        await trx("trs_alur_tanda_tangan").where("id_alur_tanda_tangan", existingFlow.id_alur_tanda_tangan).update({
          status_alur: "aktif",
          urutan_aktif: 1,
