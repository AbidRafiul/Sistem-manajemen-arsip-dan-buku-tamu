...
router.post("/", async (req, res) => {
  ...
  if (!Key) return res.status(400).json({ status: status.GAGAL, message: "Key wajib diisi" });

  try {
    const config = await DB("config").where("kode", Key).select("keterangan").first();
    if (!config) {
        return res.json({ status: status.SUKSES, data: "" });
    }
    ...
    return res.status(500).json({ status: status.GAGAL, message: err.message });
  }
});
