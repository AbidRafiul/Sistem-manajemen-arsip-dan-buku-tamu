...
router.get("/pending", async (req, res) => {
  const permissionError = await permissionGuard(req, res, pendingMenuPaths, "canView");
  if (permissionError) return permissionError;
  try {
    return await listDocuments(req, res, false);
  } catch (error) {
    await Logging(error, {
      file: "tte.js",
      func: "pending",
      request: JSON.stringify(getBasePayload(req)),
      response: "Dokumen menunggu tanda tangan gagal diambil",
      user: req?.auth?.nama_pengguna || "",
    });
    return res.status(500).json({ status: false, message: "Dokumen menunggu tanda tangan gagal diambil" });
  }
});

router.get("/signed", async (req, res) => {
  const permissionError = await permissionGuard(req, res, signedMenuPaths, "canView");
  if (permissionError) return permissionError;
  try {
    return await listDocuments(req, res, true);
  } catch (error) {
    await Logging(error, {
      file: "tte.js",
      func: "signed",
      request: JSON.stringify(getBasePayload(req)),
      response: "Dokumen tertandatangani gagal diambil",
      user: req?.auth?.nama_pengguna || "",
    });
    return res.status(500).json({ status: false, message: "Dokumen tertandatangani gagal diambil" });
  }
});

router.get("/surat-keluar/:id_surat_keluar/tanda-tangan", async (req, res) => {
  const permissionError = await permissionGuard(req, res, pendingMenuPaths, "canView");
  if (permissionError) return permissionError;
  ...
  try {
    ...
    const baseFile = await getCurrentActiveFile(payload.id_surat_keluar);
    const signatures = await getSignatureHistory(payload.id_surat_keluar);
    const verifications = await getVerificationHistory(payload.id_surat_keluar);
    const posisi = await selectSigningPosition(surat);
    const certificates = await DB("mst_sertifikat_elektronik as mse")
      .leftJoin("mst_pengguna as u", "mse.id_pengguna", "u.id_pengguna")
      .select("mse.*", "u.nama_lengkap as nama_pengguna", "u.nama_pengguna as username_pengguna")
      .where("mse.status_sertifikat", "aktif")
      .orderBy("mse.updated_at", "desc");
    ...
    return res.status(200).json({
      ...
      data: {
        surat,
        file_aktif: baseFile,
        file_aktif_url: baseDocumentUrl,
        signatures,
        verifications,
        posisi_tanda_tangan: posisi,
        certificates,
      },
    });
  } catch (error) {
    await Logging(error, {
      file: "tte.js",
      func: "detailTandaTangan",
      request: JSON.stringify(payload),
      response: "Informasi tanda tangan gagal diambil",
      user: req?.auth?.nama_pengguna || "",
    });
    return res.status(500).json({ status: false, message: "Informasi tanda tangan gagal diambil" });
  }
});
...
router.post("/surat-keluar/:id_surat_keluar/finalisasi", async (req, res) => {
  ...
  try {
    ...
    await DB.transaction(async (trx) => {
      ...
      const existingFlow = await trx("trs_alur_tanda_tangan")
        .where("id_surat_keluar", payload.id_surat_keluar)
        .first();

      if (existingFlow) {
        await trx("trs_alur_tanda_tangan")
          .where("id_alur_tanda_tangan", existingFlow.id_alur_tanda_tangan)
          .update({
            status_alur: "aktif",
            urutan_aktif: 1,
        ...
        await trx("trs_alur_tanda_tangan").insert({
          id_surat_keluar: payload.id_surat_keluar,
      ...
      await trx("trs_tracking_surat_keluar").insert({
        ...
        catatan: payload.catatan || "Dokumen final untuk tanda tangan elektronik dibuat",
        tanggal: new Date(),
    ...
    await recordSignatureLog({
      ...
      req,
      metadata: { path_file: pathFile, nama_file: fileName },
    });
    ...
    return res.status(500).json({ status: false, message: "Dokumen gagal difinalisasi" });
  }
});
...
router.get("/surat-keluar/:id_surat_keluar/riwayat-tanda-tangan", async (req, res) => {
  const permissionError = await permissionGuard(req, res, pendingMenuPaths, "canView");
  if (permissionError) return permissionError;

  const payload = { id_surat_keluar: req.params.id_surat_keluar };
  ...
  try {
    ...
    const signatures = await getSignatureHistory(payload.id_surat_keluar);
    const verifications = await getVerificationHistory(payload.id_surat_keluar);
    const logs = await DB("trs_log_tanda_tangan as tlog")
      .leftJoin("mst_pengguna as u", "tlog.id_pengguna", "u.id_pengguna")
      .select(
        "tlog.*",
        "u.nama_lengkap as nama_pengguna",
        "u.nama_pengguna as username_pengguna",
      )
      .where("tlog.id_surat_keluar", payload.id_surat_keluar)
      .orderBy("tlog.dibuat_pada", "asc");
    ...
    await Logging(error, {
      file: "tte.js",
      func: "riwayatTandaTangan",
      request: JSON.stringify(payload),
      response: "Riwayat tanda tangan gagal diambil",
      user: req?.auth?.nama_pengguna || "",
    });
    return res.status(500).json({ status: false, message: "Riwayat tanda tangan gagal diambil" });
  }
});
...
router.post("/verifikasi", upload.single("file"), async (req, res) => {
  ...
  try {
    ...
    } else if (payload.id_tanda_tangan_dokumen) {
      signatureRow = await DB("trs_tanda_tangan_dokumen")
        .where("id_tanda_tangan_dokumen", payload.id_tanda_tangan_dokumen)
        .first();
      if (!signatureRow) {
        return res.status(404).json({ status: false, message: "Dokumen tanda tangan tidak ditemukan" });
      }
      pdfBuffer = await loadObjectBuffer(signatureRow.lokasi_dokumen);
    } else if (payload.token_verifikasi) {
      signatureRow = await DB("trs_tanda_tangan_dokumen")
        .where("token_verifikasi", payload.token_verifikasi)
        .first();
      if (!signatureRow) {
        return res.status(404).json({ status: false, message: "Token verifikasi tidak ditemukan" });
      }
      pdfBuffer = await loadObjectBuffer(signatureRow.lokasi_dokumen);
    } else if (payload.id_surat_keluar) {
      const currentFile = await getCurrentActiveFile(payload.id_surat_keluar);
      if (!currentFile) {
        return res.status(404).json({ status: false, message: "Dokumen aktif tidak ditemukan" });
      }
    ...
    const verificationInsert = {
      id_surat_keluar: payload.id_surat_keluar || signatureRow?.id_surat_keluar || null,
      id_tanda_tangan_dokumen: signatureRow?.id_tanda_tangan_dokumen || payload.id_tanda_tangan_dokumen || null,
      token_verifikasi: payload.token_verifikasi || signatureRow?.token_verifikasi || null,
...
router.get("/sertifikat-elektronik", async (req, res) => {
  ...
  try {
    const payload = getBasePayload(req);
    const nPage = toPositiveNumber(payload.page, 1);
    const nLimit = Math.min(toPositiveNumber(payload.limit, 20), 100);
    const nOffset = (nPage - 1) * nLimit;
    const keyword = payload.keyword || "";

    const query = DB("mst_sertifikat_elektronik as mse")
      .leftJoin("mst_pengguna as u", "mse.id_pengguna", "u.id_pengguna")
      .select(
        "mse.*",
        "u.nama_lengkap as nama_pengguna",
    ...
    if (keyword) {
      query.where((builder) => {
        builder
          .where("mse.nama_sertifikat", "like", `%${keyword}%`)
          .orWhere("mse.alias_sertifikat", "like", `%${keyword}%`)
          .orWhere("mse.nomor_seri", "like", `%${keyword}%`)
          .orWhere("mse.subjek_sertifikat", "like", `%${keyword}%`);
      });
    }

    const totalRows = await query.clone().clearSelect().clearOrder().count({ total_data: "mse.id_sertifikat_elektronik" });
    const nTotalData = Number(totalRows?.[0]?.total_data || 0);
    ...
    return res.status(500).json({ status: false, message: "Sertifikat elektronik gagal diambil" });
  }
});

router.post("/sertifikat-elektronik", async (req, res) => {
  const permissionError = await permissionGuard(req, res, certificateMenuPaths, "canCreate");
  if (permissionError) return permissionError;
  ...
  try {
    const validation = await validatePayload(
      {
        nama_sertifikat: Joi.string().max(150).required(),
        alias_sertifikat: Joi.string().max(100).allow(null, "").optional(),
        nomor_seri: Joi.string().max(150).required(),
        subjek_sertifikat: Joi.string().max(255).allow(null, "").optional(),
        penerbit_sertifikat: Joi.string().max(255).allow(null, "").optional(),
        algoritma_tanda_tangan: Joi.string().max(50).optional(),
        algoritma_hash: Joi.string().max(50).optional(),
        lokasi_keystore: Joi.string().max(255).required(),
        berlaku_mulai: Joi.date().allow(null, "").optional(),
        berlaku_sampai: Joi.date().allow(null, "").optional(),
        status_sertifikat: Joi.string().valid("aktif", "nonaktif", "kedaluwarsa").optional(),
        id_pengguna: Joi.number().allow(null).optional(),
      },
      {
        "nama_sertifikat.required": "Nama sertifikat wajib diisi",
        "nomor_seri.required": "Nomor seri sertifikat wajib diisi",
        "lokasi_keystore.required": "Lokasi keystore wajib diisi",
      },
      payload,
      { allowUnknown: true },
    );
    ...
    const insertPayload = await buildCertificatePayload(payload, req);
    const inserted = await DB("mst_sertifikat_elektronik").insert(insertPayload);

    return res.status(201).json({
      status: true,
      message: "Sertifikat elektronik berhasil disimpan",
      data: {
        id_sertifikat_elektronik: Array.isArray(inserted) ? inserted[0] : inserted,
      },
    });
  } catch (error) {
    await Logging(error, {
      file: "tte.js",
      func: "sertifikatElektronikCreate",
      request: JSON.stringify(payload),
      response: "Sertifikat elektronik gagal disimpan",
      user: req?.auth?.nama_pengguna || "",
    });
    return res.status(500).json({
      status: false,
      message: "Sertifikat elektronik gagal disimpan",
      error: error.message,
    });
  }
});

router.put("/sertifikat-elektronik/:id_sertifikat_elektronik", async (req, res) => {
  const permissionError = await permissionGuard(req, res, certificateMenuPaths, "canUpdate");
  if (permissionError) return permissionError;

  const payload = { ...getBasePayload(req), id_sertifikat_elektronik: req.params.id_sertifikat_elektronik };

  try {
    const validation = await validatePayload(
      {
        id_sertifikat_elektronik: Joi.number().required(),
        nama_sertifikat: Joi.string().max(150).optional(),
        alias_sertifikat: Joi.string().max(100).allow(null, "").optional(),
        nomor_seri: Joi.string().max(150).optional(),
        subjek_sertifikat: Joi.string().max(255).allow(null, "").optional(),
        penerbit_sertifikat: Joi.string().max(255).allow(null, "").optional(),
        algoritma_tanda_tangan: Joi.string().max(50).optional(),
        algoritma_hash: Joi.string().max(50).optional(),
        lokasi_keystore: Joi.string().max(255).optional(),
        berlaku_mulai: Joi.date().allow(null, "").optional(),
        berlaku_sampai: Joi.date().allow(null, "").optional(),
        status_sertifikat: Joi.string().valid("aktif", "nonaktif", "kedaluwarsa").optional(),
        id_pengguna: Joi.number().allow(null).optional(),
      ...
      {
        "id_sertifikat_elektronik.required": "id_sertifikat_elektronik wajib diisi",
        "id_sertifikat_elektronik.number": "id_sertifikat_elektronik harus berupa angka",
      },
      payload,
      { allowUnknown: true },
    );
    ...
    const existing = await DB("mst_sertifikat_elektronik")
      .where("id_sertifikat_elektronik", payload.id_sertifikat_elektronik)
      .first();

    if (!existing) {
      return res.status(404).json({ status: false, message: "Sertifikat elektronik tidak ditemukan" });
    }

    const updatePayload = await buildCertificatePayload(
      { ...existing, ...payload, id_pengguna: payload.id_pengguna ?? existing.id_pengguna },
      req,
    );

    await DB("mst_sertifikat_elektronik")
      .where("id_sertifikat_elektronik", payload.id_sertifikat_elektronik)
    ...
    await Logging(error, {
      file: "tte.js",
      func: "sertifikatElektronikUpdate",
      request: JSON.stringify(payload),
      response: "Sertifikat elektronik gagal diperbarui",
      user: req?.auth?.nama_pengguna || "",
    });
    return res.status(500).json({
      status: false,
