...
const letterDispositionComplete = async (req, res) => {
  try {
    ...
    oPayload.disposisi_id = oPayload.disposisi_id || oPayload.disposisi_surat_id || oPayload.disid_jabatan || oPayload.disposition_id;
    ...
    await DB.transaction(async trx => {
      await trx("trs_disposisi_surat").where("disposisi_surat_id", oPayload.disposisi_id).update({
        status: "selesai",
      ...
      const vaUnfinishedDispositions = await trx("trs_disposisi_surat").where("surat_masuk_id", oDisposition.surat_masuk_id).whereNot("status", "selesai");
      if (vaUnfinishedDispositions.length === 0) {
        bAllDispositionCompleted = true;
        await trx("trs_surat_masuk").where("surat_masuk_id", oDisposition.surat_masuk_id).update({
          status: "selesai",
          updated_by: nActorId,
          updated_at: dNow
        });
