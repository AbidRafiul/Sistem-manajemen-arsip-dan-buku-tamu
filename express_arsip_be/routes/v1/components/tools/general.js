...
export const datetime = () => {
  const dNow = new Date();
  const datetime = dNow.getFullYear() + String(dNow.getMonth() + 1).padStart(2, "0") + String(dNow.getDate()).padStart(2, "0") + String(dNow.getHours()).padStart(2, "0") + String(dNow.getMinutes()).padStart(2, "0") + String(dNow.getSeconds()).padStart(2, "0");
  return datetime;
};
...
export const isoDateNowYmd = () => new Date().toISOString().split("T")[0].replace(/-/g, "");
...
export const uniqueId = () => Date.now().toString(36) + Math.random().toString(36).substr(2, 5);
export const getXMinutesFromNow = x => {
  const dNow = new Date();
  dNow.setMinutes(dNow.getMinutes() + x);
  return dNow;
};
...
export const generateRandomClientSecret = () => {
  ...
  randomValues.forEach(value => {
    oResult += chars[value % chars.length];
  });
  return oResult + Date.now().toString(36);
};
export const generateRandomClientUser = () => {
  const chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
  let oResult = "";
  for (let i = 0; i < 32; i++) {
    oResult += chars.charAt(Math.floor(Math.random() * chars.length));
  }
  return oResult;
};
...
const decodeHtmlEntities = str => {
  ...
  let s = str.replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&amp;/gi, "&").replace(/&quot;/gi, '"').replace(/&#x27;/gi, "'").replace(/&#x2F;/gi, "/");
